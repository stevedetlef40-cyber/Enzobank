...
class AddMoneyController extends Controller
{
    ...
    public function success(Request $request, $gateway)
    {
        try {
            ...
            if (Transaction::where('callback_ref', $token)->exists()) {
                if (! $temp_data) {
                    return redirect()->route('user.add.money.index')->with(['success' => ['Transaction failed. Record didn\'t saved properly. Please try again.']]);
                }
            } else {
                if (! $temp_data) {
                    return redirect()->route('user.add.money.index')->with(['error' => ['Transaction failed. Record didn\'t saved properly. Please try again.']]);
                }
            }
    ...
    public function showManualForm($token)
    {
        ...
        if (! $tempData || $tempData->data == null || ! isset($tempData->data->gateway_currency_id)) {
            return redirect()->route('user.add.money.index')->with(['error' => ['Invalid request']]);
        }
        ...
        if (! $gateway_currency || ! $gateway_currency->gateway->isManual()) {
            return redirect()->route('user.add.money.index')->with(['error' => ['Selected gateway is invalid']]);
        }
        $gateway = $gateway_currency->gateway;
        if (! $gateway->input_fields || ! is_array($gateway->input_fields)) {
            return redirect()->route('user.add.money.index')->with(['error' => ['This payment gateway is under constructions. Please try with another payment gateway']]);
        }
    ...
    public function manualSubmit(Request $request, $token)
    {
        ...
        if (! $amount) {
            return redirect()->route('user.add.money.index')->with(['error' => ['Transaction Failed. Failed to save information. Please try again']]);
        }
        $wallet = UserWallet::find($tempData->data->wallet_id ?? null);
        if (! $wallet) {
            return redirect()->route('user.add.money.index')->with(['error' => ['Your wallet is invalid!']]);
        }
    ...
    public function manualPreviewSubmit(Request $request)
    {
        ...
        if ($verification_otp_response instanceof RedirectResponse) {
            return $verification_otp_response;
        }
