...
class SalaryDisbursementController extends Controller
{
    ...
    public function details($username)
    {
        ...
        if (! $company) {
            return back()->with(['error' => ['Sorry! Company not found.']]);
        }
    ...
    public function store(Request $request, $username)
    {
        ...
        if ($validator->fails()) {
            return back()->withErrors($validator)->withInput()->with('modal', 'user-add');
        }
        $validated = $validator->validate();
    ...
    public function employeeList($username)
    {
        ...
        if (count($employee_lists) == 0) {
            return back()->with(['error' => ['Please add employee first!']]);
        }
    ...
    // transaction saved
    public function receiverData($user, $wallet, $amount, $salary_disbursement_id, $company)
    {
    ...
    public function updateReceiverWalletBalance($wallet, $amount)
    {
        $available_balance = $wallet->balance + $amount;
        $wallet->update([
    ...
    // insert sender data
    public function insertSenderData($user, $wallet, $amount, $salary_disbursement_id, $data)
    {
        $basic_settings = BasicSettings::first();
        $trx_id = generateTrxString('transactions', 'trx_id', 'SD-', 14);
        $available_balance = $wallet->balance - $amount;
    ...
    // update wallet balance
    public function updateSenderWalletBalance($wallet, $amount)
    {
        $available_balance = $wallet->balance - $amount;
        $wallet->update([
            'balance' => $available_balance,
