...
class MoneyOutController extends Controller
{
    ...
    public function info()
    {
        ...
        if (! $user_wallet_data) {
            return Response::error(['Sorry! Wallet not found!']);
        }
        $user_wallet = [
    ...
    public function submit(Request $request)
    {
        ...
        if (! $gateway->isManual()) {
            return Response::error([__('Gateway isn\'t available for this transaction.')], [], 400);
        }
        ...
        if ($exchange_request_amount < $gateway_min_limit || $exchange_request_amount > $gateway_max_limit) {
            return Response::error(['Please follow the transaction limit. (Min '.$gateway_min_limit.' '.$sender_wallet->currency->code.' - Max '.$gateway_max_limit.' '.$sender_wallet->currency->code.')'], [], 400);
        }
    ...
    public function instruction(Request $request)
    {
        ...
        if (! $tempData) {
            return Response::error([__('Data not found!')], [], 400);
        }

        $gateway_currency_id = $tempData->data->gateway_currency_id ?? '';
        if (! $gateway_currency_id) {
            return Response::error([__('Invalid Request!')], [], 400);
        }

        $gateway_currency = PaymentGatewayCurrency::find($gateway_currency_id);
        if (! $gateway_currency) {
            return Response::error([__('Payment gateway currency is invalid!')], [], 400);
        }
        $gateway = $gateway_currency->gateway;
        $input_fields = $gateway->input_fields;
        if ($input_fields == null || ! is_array($input_fields)) {
            return Response::error([__('This gateway is temporary pause or under maintenance!')], [], 400);
        }
    ...
    public function instructionSubmit(Request $request)
    {
        ...
        if (! $wallet) {
            return Response::error([__('Your wallet is invalid!')], [], 400);
        }

        $amount = $tempData->data->charges;

        if ($wallet->balance < $amount->total_payable) {
    ...
    public function confirm(Request $request)
    {
        ...
        if ($wallet->balance < $amount->total_payable) {
            return redirect()->route('user.money-out.index')->with(['error' => ['Your wallet balance is insufficient!']]);
        }
