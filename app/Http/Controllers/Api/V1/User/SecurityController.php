...
class SecurityController extends Controller
{
    ...
    public function getKycInputFields()
    {
        ...
        if ($user->kyc_verified == GlobalConst::VERIFIED) {
            return Response::success([__('You are already KYC Verified User')], [
        ...
        if ($user->kyc_verified == GlobalConst::PENDING) {
            return Response::success([__('Your KYC information is submitted. Please wait for admin confirmation')], [
                'instructions' => $instructions,
                'status' => $user->kyc_verified,
                'input_fields' => [],
            ], 200);
        ...
        if (! $user_kyc) {
            return Response::error([__('User KYC section is under maintenance')], [], 503);
        }
    ...
    public function KycSubmit(Request $request)
    {
        $user = auth()->guard(get_auth_guard())->user();
        if ($user->kyc_verified == GlobalConst::VERIFIED) {
            return Response::warning([__('You are already KYC Verified User')], [], 400);
        }
