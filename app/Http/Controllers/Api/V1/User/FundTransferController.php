...
class FundTransferController extends Controller
{
    ...
    public function beneficiarySelect(Request $request)
    {
        ...
        if (! array_key_exists($beneficiary->info->method->slug, $confirm_methods)) {
            return Response::error([__('Method not found')], [], 400);
        }
    ...
    public function chargeInfo(Request $request)
    {
        ...
        if (! $fees_and_charge) {
            return Response::error([__('Fess And Charge Not Defined, Please Contact With Support!')], [], 400);
        }
    ...
    public function submit(Request $request)
    {
        ...
        if (! $user_wallet) {
            return Response::error([__('Your wallet not found.')], [], 400);
        }

        if ($validated['amount'] > $user_wallet->balance) {
    ...
    // limit check
    public function transactionLimitCheck($charge_calculation, $fees_and_charge)
    {
    ...
    public function ownBankTransferConfirm(TemporaryData $temp_data)
    {
        ...
        if (! $receiver) {
            return Response::error([__('Receiver not found')], [], 400);
        }
        $receiver_wallet = UserWallet::active()->where('user_id', $receiver->id)->first();
        if (! $receiver_wallet) {
            return Response::error([__('Receiver wallet not found')], [], 400);
        }
    ...
    public function otherBankTransferSubmit($validated, $fees_and_charge, $temp_data)
    {
        ...
        $user_wallet = UserWallet::where('user_id', Auth::id())->first();
        $charge_calculation = $this->transferCharges($validated['amount'], $fees_and_charge, $user_wallet);
        if ($charge_calculation['payable'] > $user_wallet->balance) {
            return [
                'status' => false,
                'message' => 'Insufficient Balance',
            ];
