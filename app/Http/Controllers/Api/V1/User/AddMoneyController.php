...
class AddMoneyController extends Controller
{
    ...
    public function gatewayAdditionalFields(Request $request)
    {
        ...
        if (Gpay::isGpay($gateway)) {
            ...
            if ($gpay_bank_list == false) {
                return Response::error(['Gpay bank list server response failed! Please try again'], [], 500);
            }
            $data['available'] = true;
    ...
    public function cryptoPaymentConfirm(Request $request, $trx_id)
    {
        ...
        if (! isset($validated['txn_hash'])) {
            return Response::error(['Transaction hash is required for verify']);
        }
        ...
        if (! $crypto_transaction) {
            return Response::error(['Transaction hash is not valid! Please input a valid hash'], [], 404);
        }

        if ($crypto_transaction->amount >= $transaction->total_payable == false) {
            if (! $crypto_transaction) {
                Response::error(['Insufficient amount added. Please contact with system administrator'], [], 400);
            }
        }
