...
class DashboardController extends Controller
{
    public function dashboard()
    {
        ...
        $transactions = Transaction::auth()->orderByDesc('id')->latest()->get()->map(function ($data) {
            ...
            } elseif ($data->type == PaymentGatewayConst::TYPEMONEYOUT) {
                if (@$data->gateway_currency->gateway->isManual()) {
                    $type = '(Manual)';
                } else {
                    $type = '';
            ...
            } elseif ($data->type == PaymentGatewayConst::TYPE_OWN_BANK_TRANSFER) {
                return [
                    'type' => $data->type,
                    'trx_id' => $data->trx_id,
                    'request_amount' => floatval($data->request_amount),
            ...
            } elseif ($data->type == PaymentGatewayConst::TYPE_OTHER_BANK_TRANSFER) {
                return [
                    'type' => $data->type,
                    'trx_id' => $data->trx_id,
                    'request_amount' => floatval($data->request_amount),
            ...
            } elseif ($data->type == PaymentGatewayConst::TYPEVIRTUALCARD) {
                return [
                    'type' => $data->type,
                    'trx_id' => $data->trx_id,
                    'request_amount' => floatval($data->request_amount),
            ...
            } elseif ($data->type == PaymentGatewayConst::SALARYDISBURSEMENT) {
            ...
            } elseif ($data->type == PaymentGatewayConst::TYPEADDSUBTRACTBALANCE) {
                return [
