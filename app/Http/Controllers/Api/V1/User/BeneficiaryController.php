...
class BeneficiaryController extends Controller
{
    ...
    public function accountDetails(Request $request)
    {
        ...
        if ($validator->fails()) {
            return Response::error($validator->error()->all(), []);
        }
        $validated = $validator->validate();
        $user = auth()->user();
        ...
        if (! $account_details) {
            return Response::error([__('Sorry! There is no user in this account number.')], [], 400);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (! $method) {
            return Response::error(['Transaction type isn\'t available or under maintenance'], [], 400);
        }

        if ($validated['method'] == 'own-bank-transfer') {
            $receiver = User::where('account_no', $validated['account_number'])->first();
            if (! $receiver) {
                return Response::error(['Account doesn\'t exists!'], [], 404);
            }
            if (Beneficiary::auth()->where('transaction_method_id', $method->id)->whereJsonContains('info->account_number', $receiver->account_no)->exists()) {
                return Response::error(['Beneficiary already exist'], [], 400);
            }
        } else {
            ...
            if (Beneficiary::where('user_id', auth()->user()->id)->where('transaction_method_id', $method->id)->whereJsonContains('info->'.$account_col, $validated[$account_col])->whereJsonContains('info->bank_id', $validated['bank'])->exists()) {
                return Response::error(['Beneficiary already exist'], [], 400);
            }
        }
    ...
    public function delete(Request $request)
    {
        ...
        if (! $beneficiary) {
            return Response::error([__('Beneficiary data not found.')], [], 400);
        }
        try {
            $beneficiary->delete();
        } catch (Exception $e) {
