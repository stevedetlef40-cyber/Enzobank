@if (admin_permission_by_name("admin.add.money.reject"))
    @isset($transaction)
            ...
            <div id="reject-modal" class="mfp-hide large">
                <div class="modal-data">
                    <div class="modal-header px-0">
                        <h5 class="modal-title">{{ __("Rejejct Transaction") }} {{ "#" . $transaction->trx_id }}</h5>
                    </div>
                    <div class="modal-form-data">
                        <form class="modal-form" method="POST" action="{{ setRoute('admin.add.money.reject') }}" enctype="multipart/form-data">
                            @csrf
